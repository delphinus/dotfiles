...
return setmetatable({
  ...
  __call = function(self)
    ...
    local radius = math.min(
      vim.o.columns / self.pixel_ratio, vim.o.lines
    ) / 8 * 3
    ...
    local min_col, max_col =
      center_x - radius * self.pixel_ratio,
      center_x + radius * self.pixel_ratio - 1
    local min_row, max_row =
      center_y - radius * self.pixel_ratio,
      center_y + radius * self.pixel_ratio - 1
    for col = min_col, max_col do
      for row = min_row, max_row do
        -- calculate the coordinate mathematically
        local x, y =
          (col - center_x) / self.pixel_ratio,
          center_y - row
