...
# from debian (versatile version)
  ...
  delim=`rep_char $size ' '`
  side=
  headers=(40 41 42 43 44 45 46 47)
  fgs=(0 1 30 "1;30" 31 "1;31" 32 "1;32" 33 "1;33" 34 "1;34" 35 "1;35" 36 "1;36" 37 "1;37")
  if [ $mode -eq 16 ]; then
    headers+=(100 101 102 103 104 105 106 107)
    fgs+=(90 "1;90" 91 "1;91" 92 "1;92" 93 "1;93" 94 "1;94" 95 "1;95" 96 "1;96" 97 "1;97")
  fi
  if $title; then
    textlength=${#text}
    side=`rep_char $size ' '`
    ...
    for i in ${headers[@]}; do
      ...
      head="$head$delim`rep_char $((textlength - 3)) ' '`$space${i}m  "
    done
    echo
    echo "$head"
    echo "$head" | sed 's/[^|]/-/g;s/|/+/'
...
if [ $colors -eq 8 ]; then
  ...
  system_color_table2 "gYw" $((size - 1)) 16;
